...
function onePageScroll(element, options) {
  ...
  this.init = function () {
    ...
    if (settings.pagination == true) {
      ...
      var posTop =
        (document.querySelector(".onepage-pagination").offsetHeight / 2) * -1;
    ...
    if (window.location.hash != "" && window.location.hash != "#1") {
      ...
      _addClass(
        document.querySelector(settings.sectionContainer + "[data-index='1']"),
        "active"
      );
      _addClass(body, "viewing-page-1");
      if (settings.pagination == true)
        _addClass(
          document.querySelector(".onepage-pagination li a[data-index='1']"),
          "active"
        );
    }
    ...
    if (settings.pagination == true) {
      var pagination_links = document.querySelectorAll(
  ...
  _transformPage = function (el2, settings, pos, index, next_el) {
    if (typeof settings.beforeMove == "function")
      settings.beforeMove(index, next_el);
    ...
    function endAnimation() {
      if (typeof settings.afterMove == "function")
        settings.afterMove(index, next_el);
  ...
  this.moveDown = function (el3) {
    ...
    var index = document.querySelector(settings.sectionContainer + ".active")
        .dataset.index,
    ...
    if (settings.pagination == true) {
      ...
      _addClass(
        document.querySelector(
          ".onepage-pagination li a" + "[data-index='" + next_index + "']"
        ),
        "active"
      );
    }
  ...
  this.moveUp = function (el4) {
    ...
    if (!next) {
      if (settings.loop == true) {
        pos = (total - 1) * 100 * -1;
        next = document.querySelector(
          settings.sectionContainer + "[data-index='" + total + "']"
        );
      } else {
        return;
      }
  ...
  this.moveTo = function (el5, page_index) {
    ...
    if (next) {
      ...
      _addClass(next, "active");
      _removeClass(
        document.querySelector(".onepage-pagination li a" + ".active"),
        "active"
      );
      _addClass(
        document.querySelector(
...
onePageScroll(".main", {
  ...
  responsiveFallback: false, // You can fallback to normal page scroll by defining the width of the browser in which                                  // you want the responsive fallback to be triggered. For example, set this to 600 and whenever
